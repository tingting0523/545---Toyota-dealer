...
def sale_data_serialize():
    try:
        ...
        if connection.is_connected():
            ...
            sale_data = sale_data_insert(cursor)
            
            event_json = json.dumps(sale_data) # serialize the data to an event.
            connection.commit()
            producer.produce("car_db", event_json.encode('utf-8'), callback=acked)
            producer.flush()
            
            print(f"{sale_data['cust_wholename']} purchased a {sale_data['color']} "
...
def maint_data_serialize():
    try:
        ...
        if connection.is_connected():
            ...
            maint_data = maint_data_insert(cursor)
